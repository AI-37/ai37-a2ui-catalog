...
action'ов и сетевых вызовов (кроме подсказок lookup): раскрытие/сворачивание
карточек, правку типа/subtype/названия, правку строк слоёв (материал,
толщина, λ), добавление и удаление строк слоя, добавление и удаление
конструкций, ввод `rprPassport` для типов с `hasLayers: false` (вместо
таблицы слоёв). До нажатия submit или back компонент SHALL NOT вызывать
`dispatchAction`.

#### Scenario: Добавление и удаление слоя

- **WHEN** пользователь нажимает «+ Слой», заполняет строку и затем удаляет
  другую строку корзинкой
- **THEN** таблица обновляется мгновенно, `dispatchAction` не вызывался,
  сетевых запросов кроме подсказок не было

#### Scenario: Тип без слоёв

- **WHEN** тип карточки меняется на тип с `hasLayers: false` (окно/дверь)
- **THEN** вместо таблицы слоёв рендерится одно поле «Rпр по паспорту»
