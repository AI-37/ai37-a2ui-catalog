...
строкой-сводкой (`meta`) и раскрывать полный `content`. При заданном
`downloadFileName` в строке-сводке SHALL быть кнопка «Скачать», отдающая
`content` клиентским Blob'ом под этим именем файла, не раскрывая протокол;
без `downloadFileName` кнопки нет.
...
- **WHEN** пользователь нажимает «Скачать» при заданном `downloadFileName`
- **THEN** браузер сохраняет content файлом с этим именем, протокол при этом
  не раскрывается
