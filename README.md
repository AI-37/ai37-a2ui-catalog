...
TypeScript, React 19, Zod (zod-to-json-schema), @a2ui/react / @a2ui/web_core (overrides: 0.10.1), @base-ui/react ^1.7.0 (поведение UI), katex, Vite, Vitest, tsup, tsx, Python 3.13+ + Pydantic, Poetry 2.3.2, Twine, pnpm (>=10, packageManager pnpm@10.29.3), Node >=22. Версия пакетов workspace — 0.35.0. Публикация пакетов — в приватные реестры AI-37 (npm.app.sp-ai.ru и pypi.app.sp-ai.ru). Константы дебаунса автодрафта условий: `CONDITIONS_DRAFT_DEBOUNCE_MS = 500` мс (@ai37/a2ui-catalog-react), `LIFT_DRAFT_DEBOUNCE_MS = 500` мс.
...
Коэффициент теплотехнической однородности r (Г.4 СП 50.13330.2024; change `constructions-editor-r`, план `thermal-r-uniformity-sp230`, этап A): `ConstructionEntry.r` — опциональное число (0; 1] (`z.number().gt(0).max(1).optional()` в zod-схеме и `r: float = Field(default=None, gt=0, le=1)` в Python-зеркале). Отсутствие ключа означает «r не задан»: клиент значение по умолчанию не подставляет — агент считает с r = 1 и помечает это допущением в отчёте. Поле «r (однородность)» (`R_FIELD_LABEL === 'r (однородность)'`) живёт в форме шапки карточки `ConstructionsEditorNext` (`constructions-next-r-field.tsx`) и показывается только у типов с `hasLayers`; как и тип с названием, значение уходит наверх только по «Сохранить» (`headerFieldsEqual` сравнивает r), смена типа на изделие (окна/двери/фонари) сбрасывает r (`next-header-draft-for-type.ts`), очищенное поле снимает ключ с конструкции (`withHeaderFields` — не оставляет `undefined`-значение, чтобы payload черновика и submit был буквально равен состоянию). Контрол — `NumberField` с `min=0.01`, `max=1`, `step=0.01` и плейсхолдером «1» (подсказка, не значение); `NumberField` получил оси `max` (парную к `min`) и `placeholder`. Сводка шапки печатает `· r = 0.85` (`toFixed(2)`, разделитель приклеен неразрывным пробелом) — строки нет, когда r не задан. Live-Rпр слоистой конструкции считается как `Rпр = r · (1/αв + 1/αн + Σ δ/λ)`, без r — на 1 (`computeLiveRpr`). Старое семейство `ConstructionsEditor` поле не показывает, но присланный r сохраняет (его форма шапки ключа не шлёт, spread коммита оставляет `entry.r` нетронутым).
...
- packages/catalog-schemas — канонические Zod-схемы, типы и генерация JSON Schema (включая `components/keo-editor.ts`, `components/thermal-report.ts`, `components/lift-report.ts`, `components/constructions-editor.ts` с полем `r` у `ConstructionEntry` и регистрацию в `CATALOG_COMPONENT_NAMES`, `constants.ts`, `catalog.ts`, `index.ts`);
- packages/catalog-react — React-рендереры компонентов каталога (в т.ч. `renderers/keo-editor-next.tsx`, `renderers/use-keo-draft-post.ts`, `renderers/apply-keo-draft-seed.ts`, `renderers/thermal-report*.tsx/.ts`, `renderers/lift-report*.tsx/.ts`, `thermal-report-protocol.tsx`, `lift-report-protocol.tsx`, `renderers/find-invalid-layers.ts`, `renderers/is-subtype-required.ts`, `renderers/constructions-next-header-summary.tsx`, `renderers/constructions-next-header-form.tsx`, `renderers/constructions-next-r-field.tsx`, `renderers/next-header-draft-for-type.ts`, `renderers/with-header-fields.ts`, `renderers/header-fields-equal.ts`, `renderers/compute-live-rpr.ts`, группы токенов `tr`/`lr`/`le`/`ce` в `tokens.ts`, общие стили и контролы полей — `shared.ts`, `calc-editor-field.tsx`, `lift-editor-field.tsx`, `constructions-editor-styles.ts`) и подпуть `primitives/` (примитивы набора, включая `number-field.tsx` с осями `min`/`max`/`step`/`placeholder`);
- packages/catalog-python — Pydantic-модели валидации (зеркало zod-схем; `draftUrl` в `models/keo_editor.py`, `r` в `ConstructionEntry` модели `models/constructions_editor.py`);
- fixtures — валидные, невалидные и сквозные фикстуры A2UI-сообщений (включая `valid/thermal-report-single.json`, `valid/thermal-report-multi.json`, `valid/lift-report.json`, `valid/keo-editor-draft.json`, `valid/constructions-editor-r.json`, `invalid/keo-editor-absolute-draft-url.json`, `invalid/constructions-editor-r-above-one.json`, `messages/constructions-editor-surface.json`);
- tests — тесты (tests/react — Vitest, включая `keo-editor-next.test.tsx`, `constructions-editor-next.test.tsx`, `next-header-draft-for-type.test.ts`, `find-invalid-layers.test.ts`, `thermal-report.test.tsx`, `lift-report.test.tsx`, `constructions-editor.test.tsx`, `lookup-option-rich-render.test.tsx`; tests/ts — включая `catalog-schemas.test.ts`, `keo-editor-schema.test.ts`, `thermal-report-schema.test.ts`, `lift-report-schema.test.ts`, `parse-lookup-options.test.ts`; python-часть — Pytest, `tests/python/test_models.py`);
- public/a2ui/catalogs — статические артефакты каталога (catalog.json, JSON Schema компонентов, включая `components/keo-editor.schema.json`, `components/keo-editor-next.schema.json`, `components/thermal-report.schema.json`, `components/lift-report.schema.json`, `components/constructions-editor.schema.json` и `components/constructions-editor-next.schema.json` с полем `r`) для GitHub Pages;
...
- docs, openspec — документация и design-доки (включая openspec/changes/constructions-editor-r, openspec/changes/constructions-missing-subtype-invalid, openspec/changes/thermal-report, openspec/changes/lift-report, openspec/changes/keo-editor и прежние changes).
...
Скоуп @ai37 закреплён за npm.app.sp-ai.ru в .npmrc (always-auth=true). Версии пакетов синхронизируются через `pnpm run version:bump <version>` (текущая версия — 0.35.0); каждый PR также обновляет CHANGELOG.md. Доступные npm-скрипты: pnpm run build, typecheck, test, test:ts, test:python, export:schemas, export:public, verify:public, lint, demo, version:bump, install:consumer. Константы `CONDITIONS_DRAFT_DEBOUNCE_MS = 500` мс и `LIFT_DRAFT_DEBOUNCE_MS = 500` мс экспортируются из @ai37/a2ui-catalog-react (не env; используются тестами и хостами для единого окна дебаунса). Конфигурация: package.json (включая overrides), tsconfig.base.json, vitest.config.ts, vite.config.ts, pyproject.toml. Тематизация рендереров — через CSS-переменные (tokens.ts), включая токены групп `tr`/`lr`/`le`/`ce` и общие статусные danger/success/warning.

## Данные и хранилища

БД, Redis и S3 отсутствуют. Статические артефакты каталога: public/a2ui/catalogs/ai37-a2ui/v1 и v2. В 0.35.0 перегенерированы `catalog.json` и `components/constructions-editor.schema.json` / `components/constructions-editor-next.schema.json` — в `ConstructionEntry` добавлено опциональное `r` (`type: number`, `exclusiveMinimum: true`, `minimum: 0`, `maximum: 1`); zod-схема и Python-зеркало обновлены вместе с ними (`CATALOG_VERSION` v2 не менялся — набор расширяется аддитивно). Ранее в v2 уже лежат `components/thermal-report.schema.json`, `components/lift-report.schema.json`, `components/keo-editor.schema.json`, `components/keo-editor-next.schema.json` (у KeoEditor/KeoEditorNext — optional `draftUrl` со строго относительным путём; у LiftReport JSON Schema отражает strict-контракт с обязательными `verdict` и `inputs`). Фикстуры: fixtures/valid (включая thermal-report-single.json, thermal-report-multi.json — в protocol `content` и `downloadContent`; lift-report.json — с `verdict`, `suggestions`, `inputs`, `protocol` c `downloadUrl`; keo-editor-draft.json; constructions-editor-r.json — стена с вентфасадом и `r: 0.85`), fixtures/invalid (включая keo-editor-absolute-draft-url.json и constructions-editor-r-above-one.json — `r: 1.2`), fixtures/messages (constructions-editor-surface.json).
...
- `pnpm run test:ts` — только TypeScript/React-тесты (включая keo-editor-next.test.tsx, constructions-editor-next.test.tsx, next-header-draft-for-type.test.ts, find-invalid-layers.test.ts, constructions-editor.test.tsx, lookup-option-rich-render.test.tsx, thermal-report.test.tsx, lift-report.test.tsx, catalog-schemas.test.ts, keo-editor-schema.test.ts, thermal-report-schema.test.ts, lift-report-schema.test.ts, parse-lookup-options.test.ts);
- `pnpm run test:python` — только Python-тесты (`tests/python/test_models.py` прогоняет валидные и невалидные фикстуры, включая constructions-editor-r.json);
- `pnpm run lint` — typecheck.

## Деплой

GitHub Actions:
- .github/workflows/pages.yml публикует статические артефакты (включая обновлённые v2 с constructions-editor.schema.json, constructions-editor-next.schema.json, keo-editor.schema.json, keo-editor-next.schema.json, thermal-report.schema.json и lift-report.schema.json) на GitHub Pages;
