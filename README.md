...
Невалидность карточки конструкции вычисляется на клиенте в `renderers/find-invalid-layers.ts` (`ConstructionInvalidity`, считается на каждый рендер, как live-Rпр): это индикация, не блок — submit уходит как есть. Пять причин: слой без λ, слой без положительной толщины, пустое название материала, тип без слоёв без введённого паспортного `rprPassport` (`missingPassport`) и незаполненная разновидность у типа, которому она обязательна (`missingSubtype`). Обязательность разновидности выводится из конфига типа, присланного агентом (`renderers/is-subtype-required.ts`: `typeof config?.alphaN === 'object'` — αн задан записью по разновидностям), а не из списка типов в клиенте — та же механика, что у паспортного Rпр из `hasLayers`. При `missingSubtype` поле `missingLambdaCount` обнуляется: счёт «N слоёв без λ» не показывается, даже если слои заполнены полностью, — чип говорит общее «проверить», карточка получает красную рамку (`a2ui-card--invalid`). В шапке раскрытой карточки `ConstructionsNextHeaderSummary` вместо значения печатает « · разновидность не выбрана» предупреждающим оформлением (`a2ui-t--warning`), разделитель приклеен к тексту неразрывным пробелом (`' ·\u00A0разновидность не выбрана'`), у типов без разновидностей — ни текста, ни разделителя. Текст добавлен только семейству `*Next`; старое семейство получает рамку и чип общим путём через `findInvalidLayers`.

Коэффициент теплотехнической однородности r (Г.4 СП 50.13330.2024; change `constructions-editor-r`, план `thermal-r-uniformity-sp230`, этап A): `ConstructionEntry.r` — опциональное число (0; 1] (`z.number().gt(0).max(1).optional()` в zod-схеме и `r: float = Field(default=None, gt=0, le=1)` в Python-зеркале). Отсутствие ключа означает «r не задан»: клиент значение по умолчанию не подставляет — агент считает с r = 1 и помечает это допущением в отчёте. Поле «r (однородность)» живёт в форме шапки карточки `ConstructionsEditorNext` и показывается только у типов с `hasLayers`; как и тип с названием, значение уходит наверх только по «Сохранить» (`headerFieldsEqual` сравнивает r), смена типа на изделие (окна/двери/фонари) сбрасывает r (`next-header-draft-for-type.ts`), очищенное поле снимает ключ с конструкции (`withHeaderFields`). Контрол — `NumberField` с `min=0.01`, `max=1`, `step=0.01` и плейсхолдером «1» (подсказка, не значение). Сводка шапки печатает `· r = 0.85` (`toFixed(2)`); строки нет, когда r не задан. Live-Rпр слоистой конструкции считается как `Rпр = r · (1/αв + 1/αн + Σ δ/λ)`, без r — на 1 (`computeLiveRpr`).

Температура помещения по конструкции и поправка nt (формула (5.3) СП 50.13330.2024; change `constructions-editor-nt`, план `thermal-nt-room-temperature`):
- контракт — `ConstructionEntry.tvRoom` (tв* — средняя температура самого помещения, °C) и `ConstructionEntry.totRoom` (tот* — средняя температура с холодной стороны конструкции, °C; у перекрытия над подвалом это воздух подвала, а не наружный), оба опциональные числа без диапазона (`tvRoom: z.number().optional()`, `totRoom: z.number().optional()`; Python: `tvRoom: float = None`, `totRoom: float = None`). Незаданное выражается отсутствием ключа: strict-схема отвергает явный `null` (`fixtures/invalid/constructions-editor-tv-room-null.json`). Осмысленность пары схема не проверяет;
- расчёт — чистые функции `renderers/compute-nt.ts` и `renderers/effective-rnorm.ts`: `nt = (tв* − tот*) / (tв − tот)`, `Rнорм = Rтр · nt`. Незаданная температура берётся общей по зданию (`ConstructionsClimateBase = {tv, tot}` из блока «Условия»), поэтому одно введённое значение уже даёт поправку; `computeNt` возвращает `null`, когда не задана ни одна температура конструкции, когда климат здания неполон (`tv`/`tot` — null) и когда пара не даёт положительного результата (знаменатель ≤ 0 либо nt ≤ 0). Клиентской блокировки нет — о негодной паре скажет агент, канон расчёта на сервере; чипа при `null` нет;
- норма сравнения — `effectiveRnorm(config.rnorm, nt)`: базовое `rnorm` типа есть значение при nt = 1 (тип, а не конструкция), поправку по конструкции накладывает клиент. По норме конструкции считает и чип Rпр карточки, и счётчик «проходит N из M» в `constructions-editor-next.tsx`;
- чип — `renderers/constructions-next-nt-chip.tsx`: `Chip tone="neutral"` с `nt.toFixed(2)`, рендерится рядом с чипом Rпр (обёртка в `constructions-next-card.tsx`), только при вычислимой поправке;
- поля формы шапки `ConstructionsEditorNext` — `renderers/constructions-next-room-temp-field.tsx` (`Field` + `NumberField`, `step={0.1}`, без границ) показываются у ЛЮБОГО типа, включая окна и двери: nt правит норму таблицы 3, а не Rпр, и этим отличается от `r`. Плейсхолдер — значение здания (`climate.tv` / `climate.tot`), в контрол значение по умолчанию не подставляется: пустое поле читается как «как у здания». Правки уходят наверх только по «Сохранить» (`headerFieldsEqual` сравнивает `tvRoom`/`totRoom`), очищенное поле снимает ключ с конструкции (`withHeaderFields`), смена типа на изделие температуры сохраняет (в отличие от `r`, `next-header-draft-for-type.ts`). Сводка шапки печатает `· t в +16 °C` / `· t от +8 °C` (`constructions-next-header-summary.tsx`), с неразрывным пробелом-разделителем и «+» у положительных значений; не задано — строки нет. `ConstructionsClimateBase` прокинут из `ConstructionsEditorNext` в список и карточку. Схема props общая со старым рендерером — карточки с `tvRoom`/`totRoom` она принимает и там.
...
- packages/catalog-schemas — канонические Zod-схемы, типы и генерация JSON Schema (включая `components/keo-editor.ts`, `components/thermal-report.ts`, `components/lift-report.ts`, `components/constructions-editor.ts` с полем `r` и температурными полями `tvRoom`/`totRoom` у `ConstructionEntry` и регистрацию в `CATALOG_COMPONENT_NAMES`, `constants.ts`, `catalog.ts`, `index.ts`);
- packages/catalog-react — React-рендереры компонентов каталога (в т.ч. `renderers/keo-editor-next.tsx`, `renderers/use-keo-draft-post.ts`, `renderers/apply-keo-draft-seed.ts`, `renderers/thermal-report*.tsx/.ts`, `renderers/lift-report*.tsx/.ts`, `thermal-report-protocol.tsx`, `lift-report-protocol.tsx`, `renderers/find-invalid-layers.ts`, `renderers/is-subtype-required.ts`, `renderers/constructions-next-header-summary.tsx`, `renderers/constructions-next-header-form.tsx`, `renderers/constructions-next-r-field.tsx`, `renderers/constructions-next-room-temp-field.tsx`, `renderers/constructions-next-nt-chip.tsx`, `renderers/compute-nt.ts`, `renderers/effective-rnorm.ts`, `renderers/next-header-draft-for-type.ts`, `renderers/with-header-fields.ts`, `renderers/header-fields-equal.ts`, `renderers/compute-live-rpr.ts`, группы токенов `tr`/`lr`/`le`/`ce` в `tokens.ts`, общие стили и контролы полей — `shared.ts`, `calc-editor-field.tsx`, `lift-editor-field.tsx`, `constructions-editor-styles.ts`) и подпуть `primitives/` (примитивы набора, включая `number-field.tsx` с осями `min`/`max`/`step`/`placeholder`);
- packages/catalog-python — Pydantic-модели валидации (зеркало zod-схем; `draftUrl` в `models/keo_editor.py`, `r`/`tvRoom`/`totRoom` в `ConstructionEntry` модели `models/constructions_editor.py`);
- fixtures — валидные, невалидные и сквозные фикстуры A2UI-сообщений (включая `valid/thermal-report-single.json`, `valid/thermal-report-multi.json`, `valid/lift-report.json`, `valid/keo-editor-draft.json`, `valid/constructions-editor-r.json`, `valid/constructions-editor-nt.json` — стена лестничной клетки с `tvRoom: 16` и перекрытие над неотапливаемым подвалом с `totRoom: 8`, `invalid/keo-editor-absolute-draft-url.json`, `invalid/constructions-editor-r-above-one.json`, `invalid/constructions-editor-tv-room-null.json` — `tvRoom: null`, `messages/constructions-editor-surface.json`);
- tests — тесты (tests/react — Vitest, включая `compute-nt.test.ts`, `keo-editor-next.test.tsx`, `constructions-editor-next.test.tsx`, `next-header-draft-for-type.test.ts`, `find-invalid-layers.test.ts`, `thermal-report.test.tsx`, `lift-report.test.tsx`, `constructions-editor.test.tsx`, `lookup-option-rich-render.test.tsx`; tests/ts — включая `catalog-schemas.test.ts`, `keo-editor-schema.test.ts`, `thermal-report-schema.test.ts`, `lift-report-schema.test.ts`, `parse-lookup-options.test.ts`; python-часть — Pytest, `tests/python/test_models.py`);
- public/a2ui/catalogs — статические артефакты каталога (catalog.json, JSON Schema компонентов, включая `components/keo-editor.schema.json`, `components/keo-editor-next.schema.json`, `components/thermal-report.schema.json`, `components/lift-report.schema.json`, `components/constructions-editor.schema.json` и `components/constructions-editor-next.schema.json` с полями `r`, `tvRoom`, `totRoom`) для GitHub Pages;
...
- docs, openspec — документация и design-доки (включая openspec/changes/constructions-editor-nt, openspec/changes/constructions-editor-r, openspec/changes/constructions-missing-subtype-invalid, openspec/changes/thermal-report, openspec/changes/lift-report, openspec/changes/keo-editor и прежние changes).
...
- JSON Schema компонентов: .../a2ui/catalogs/ai37-a2ui/v2/components/*.schema.json (включая `keo-editor.schema.json`, `keo-editor-next.schema.json`, `thermal-report.schema.json`, `lift-report.schema.json`, `constructions-editor.schema.json` и `constructions-editor-next.schema.json`) и аналогично для v1.
...
Редакторы `ConstructionsEditor`/`ConstructionsEditorNext` и `LiftEditor`/`LiftEditorNext` принимают прежние схемы props (`constructionsEditorPropsSchema`, `liftEditorPropsSchema`) — контракт данных не менялся, Next-версии регистрируются рядом со старыми. Карточка конструкции считается невалидной (рамка `a2ui-card--invalid` + чип «проверить»), в том числе при невыбранной разновидности у типа, которому она обязательна; признак обязательности — `alphaN`, заданный записью по разновидностям в конфиге типа от агента, — тот же, что используется для невалидности карточки. В шапке раскрытой карточки `*Next` вместо значения печатается « · разновидность не выбрана». Рядом со старыми зарегистрированы и рендереры `ThermalReportNext`/`LiftReportNext` — те же схемы props и контракт действий (`{event: {name, context: payload ?? {}}}`), своих листов стилей у них нет (общий набор примитивов).

`ConstructionEntry.r` (с 0.35.0) — опциональное число в диапазоне (0; 1] (`z.number().gt(0).max(1).optional()`, Pydantic `r: float = Field(default=None, gt=0, le=1)`); значения вне диапазона (например `r: 1.2` или `r: 0`) отвергаются схемой и её Python-зеркалом. Отсутствие `r` — «не задан», клиент значение по умолчанию не подставляет. Поле «r (однородность)» показывает только форма шапки `ConstructionsEditorNext` и только у типов с `hasLayers`; у изделий (окна/двери/фонари) поля нет, смена типа на изделие сбрасывает r, очищенное поле снимает ключ `r` с конструкции; сводка шапки печатает `· r = 0.85`; live-Rпр слоистой конструкции — сумма по (Г.6), умноженная на `r ?? 1` (канонический расчёт остаётся на сервере). Прежний `ConstructionsEditor` поле не показывает, но присланный `r` в карточке сохраняет.

`ConstructionEntry.tvRoom` и `ConstructionEntry.totRoom` (с 0.36.0) — опциональные числа без диапазона (`z.number().optional()`; Python-зеркало `tvRoom: float = None`, `totRoom: float = None`): tв* — средняя температура самого помещения, tот* — средняя температура с холодной стороны конструкции. Незаданное выражается отсутствием ключа, явный `null` отвергается strict-схемой и её Python-зеркалом. Клиент выводит поправку nt по (5.3) СП 50.13330.2024 — `nt = (tв* − tот*) / (tв − tот)`, беря незаданную температуру из `general.tv`/`general.tot`, — показывает её чипом `nt 0.82` рядом с чипом Rпр и сравнивает Rпр с нормой ЭТОЙ конструкции (`Rтр · nt`); счётчик «проходит N из M» считает так же. Поля температур показывает форма шапки `ConstructionsEditorNext` у любого типа, включая окна и двери; плейсхолдер — значение здания, дефолт в контрол не подставляется. Канонический расчёт остаётся на сервере.
...
Скоуп @ai37 закреплён за npm.app.sp-ai.ru в .npmrc (always-auth=true). Версии пакетов синхронизируются через `pnpm run version:bump <version>` (текущая версия — 0.36.0); каждый PR также обновляет CHANGELOG.md. Доступные npm-скрипты: pnpm run build, typecheck, test, test:ts, test:python, export:schemas, export:public, verify:public, lint, demo, version:bump, install:consumer. Константы `CONDITIONS_DRAFT_DEBOUNCE_MS = 500` мс и `LIFT_DRAFT_DEBOUNCE_MS = 500` мс экспортируются из @ai37/a2ui-catalog-react (не env; используются тестами и хостами для единого окна дебаунса). Конфигурация: package.json (включая overrides), tsconfig.base.json, vitest.config.ts, vite.config.ts, pyproject.toml. Тематизация рендереров — через CSS-переменные (tokens.ts), включая токены групп `tr`/`lr`/`le`/`ce` и общие статусные danger/success/warning.

## Данные и хранилища

БД, Redis и S3 отсутствуют. Статические артефакты каталога: public/a2ui/catalogs/ai37-a2ui/v1 и v2. В 0.36.0 перегенерированы `catalog.json` и `components/constructions-editor.schema.json` / `components/constructions-editor-next.schema.json` — в `ConstructionEntry` добавлены опциональные `tvRoom` и `totRoom` (`type: number`, без диапазона); zod-схема и Python-зеркало обновлены вместе с ними (`CATALOG_VERSION` v2 не менялся — набор расширяется аддитивно). В 0.35.0 там же добавлено опциональное `r` (`type: number`, `exclusiveMinimum: true`, `minimum: 0`, `maximum: 1`). Ранее в v2 уже лежат `components/thermal-report.schema.json`, `components/lift-report.schema.json`, `components/keo-editor.schema.json`, `components/keo-editor-next.schema.json` (у KeoEditor/KeoEditorNext — optional `draftUrl` со строго относительным путём; у LiftReport JSON Schema отражает strict-контракт с обязательными `verdict` и `inputs`). Фикстуры: fixtures/valid (включая thermal-report-single.json, thermal-report-multi.json — в protocol `content` и `downloadContent`; lift-report.json — с `verdict`, `suggestions`, `inputs`, `protocol` c `downloadUrl`; keo-editor-draft.json; constructions-editor-r.json — стена с вентфасадом и `r: 0.85`; constructions-editor-nt.json — стена лестничной клетки с `tvRoom: 16` и перекрытие над неотапливаемым подвалом с `totRoom: 8` при климате здания `tv: 20`, `tot: -2.2`), fixtures/invalid (включая keo-editor-absolute-draft-url.json, constructions-editor-r-above-one.json — `r: 1.2`, constructions-editor-tv-room-null.json — `tvRoom: null`), fixtures/messages (constructions-editor-surface.json).
...
- `pnpm run test:ts` — только TypeScript/React-тесты (включая compute-nt.test.ts — числа приложения К СП 50 13330.2024 и поведение `effectiveRnorm`, keo-editor-next.test.tsx, constructions-editor-next.test.tsx — плейсхолдеры температур, ввод tв*/tот*, чип nt, поля у изделия, очистка, next-header-draft-for-type.test.ts, find-invalid-layers.test.ts, constructions-editor.test.tsx, lookup-option-rich-render.test.tsx, thermal-report.test.tsx, lift-report.test.tsx, catalog-schemas.test.ts — включая приём `constructions-editor-nt.json` и отказ `constructions-editor-tv-room-null.json`, keo-editor-schema.test.ts, thermal-report-schema.test.ts, lift-report-schema.test.ts, parse-lookup-options.test.ts);
- `pnpm run test:python` — только Python-тесты (`tests/python/test_models.py` прогоняет валидные и невалидные фикстуры, включая constructions-editor-r.json, constructions-editor-nt.json и constructions-editor-tv-room-null.json);
- `pnpm run lint` — typecheck.

## Деплой

GitHub Actions:
- .github/workflows/pages.yml публикует статические артефакты (включая обновлённые v2 с constructions-editor.schema.json, constructions-editor-next.schema.json — с полями `tvRoom`/`totRoom`, keo-editor.schema.json, keo-editor-next.schema.json, thermal-report.schema.json и lift-report.schema.json) на GitHub Pages;
...
- openspec/changes/constructions-editor-nt/design.md, proposal.md, specs/constructions-nt/spec.md, tasks.md — температура помещения по конструкции и поправка nt (5.3 СП 50.13330.2024), план `thermal-nt-room-temperature`, парный change агента `construction-nt`;
- openspec/changes/constructions-editor-r/design.md, proposal.md, specs/constructions-r/spec.md, tasks.md — коэффициент однородности r (Г.4 СП 50.13330.2024), план `thermal-r-uniformity-sp230` (этап A);
...
- openspec/changes/constructions-missing-subtype-invalid/design.md, proposal.md, specs/constructions-invalidity/spec.md, tasks.md, preview/README.md;
