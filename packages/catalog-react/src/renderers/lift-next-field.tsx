...
export function LiftNextField({field, value, options, sources, note, onChange}: LiftNextFieldProps) {
  return (
    <Field label={field.label}>
      <LiftNextFieldControl field={field} value={value} options={options} onChange={onChange} />
      <LiftNextFieldNote source={sources[field.name]} hint={field.hint} note={note} />
    </Field>
  );
}
