 ...
 * без рендера (моноширинный, со скроллом). «Скачать» (при `downloadFileName`)
 * отдаёт content клиентским Blob'ом — агент и транспорт не участвуют.
 */
export function ThermalReportProtocolCard({protocol}: {protocol: ThermalReportProtocol}) {
  const handleDownload = (event: React.MouseEvent) => {
    ...
    const blob = new Blob([protocol.content], {type: 'text/markdown;charset=utf-8'});
