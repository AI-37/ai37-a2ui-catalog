...
export const liftEditorPropsSchema = z
  .object({
    ...
    // Ревизия документа агента. Компонент её не читает и не показывает —
    // только возвращает без изменений в payload submit'а и черновика. По ней
    // агент отличает действие живой формы от действия устаревшего экземпляра,
    // оставшегося в истории чата, и не даёт второму затереть документ. Без
    // пропа эха нет — payload прежний.
    docRev: z.number().int().min(0).optional(),
  })
  .strict()
