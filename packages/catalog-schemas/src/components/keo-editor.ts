...
export const keoEditorPropsSchema = z
  .object({
    ...
    // клиентов. Канал реализует ТОЛЬКО `KeoEditorNext`; прежний `KeoEditor`
    // ключ игнорирует (устаревший рендерер канал не получит) — агент, которому
    // нужен REST-черновик, адресует Next. Строго относительный same-origin путь
    // (один ведущий `/`): значение уходит в fetch, и абсолютный или
