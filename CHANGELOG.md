...
- `ConstructionsEditor`: подпись у submit'а больше не печатает «проходит 0 из 0».
  Три случая разведены явно (`passing-summary.ts`): конструкций нет — «конструкций
  пока нет»; нормы нет ни у одной — «норма не задана — сравнивать не с чем»;
  норма есть у части набора — «проходит N из M · для K нормы нет».

  Мотив — дефект прода: подвал показывал «проходит 0 из 0» при трёх заполненных
  конструкциях с уже посчитанным live-Rпр. Формально честно (агент не прислал
  `rnorm`), но читается как приговор расчёту. Формулировка агностична к эмитенту:
  почему нормы нет — знает агент, он же говорит это своим текстом.

  Заодно перестал умалчивать остаток: при конфиге типа только для стен сводка
  показывала «проходит 1 из 1», пряча две конструкции из трёх.
