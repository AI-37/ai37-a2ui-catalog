...
- `ConstructionsEditor`: физические диапазоны ручного ввода в подсветке
  невалидного слоя. λ, введённая вручную, вне 0,02…5 Вт/(м·°C) и толщина сверх
  2000 мм помечают карточку «! проверить», а строка слоя называет причину
  («вне диапазона»). Значения из справочника (`lambdaA`/`lambdaB`) под проверку
  не попадают: они нормативные по определению — гранит 3,49 и сталь 58 законны.

  Мотив — дефект прода: штукатурка с λ 0,001 Вт/(м·°C) (в двадцать раз лучше
  аэрогеля) давала R 10,16 и зелёный вердикт «соответствует». Предикаты
  (`isLambdaManualOutOfRange`, `isThicknessOutOfRange`) экспортируются из
  `find-invalid-layers` и используются и подсветкой карточки, и строкой слоя —
  одни правила на оба места. Индикация, не блок: submit уходит как есть, агент
  остаётся валидатором истины.
