...
- Новые опциональные пропы: `general.gsop` (ГСОП считает агент, клиент только
  показывает его рядом с регионом и прячет, как только тронут климат),
  `generalSources` (источник каждого значения `general`: `project` /
  `question` / `suggested` / `default` с необязательным обоснованием),
  `headerTitle` и `headerContext` — шапка карточки.
...
- Подписи полей условий с макета: «Тип здания» → «Назначение помещений»,
  ...
  эксплуатации»; tот/zот/tн переехали в отдельный ряд «Климат» того же блока и
  остаются редактируемыми и частью payload'а.
